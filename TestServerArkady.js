    ...
    function (accessToken, refreshToken, profile, done) {
        ...
        client.query(query_string, values, (err, result) => {
            ...
            if (result.rows.length) {
                ...
                console.log("This user is registered: ", result.rows); // change this to flash message
...
app.post("/register", function (req, res, next) {
    ...
    //    if (!usersBeforeConfirmation.some(e => e.email === user.email) && (promoCodes.some(e => e.promoCode === user.promoCode) || user.promoCode === '' || user.promoCode === null)) {
    //        const client = new Client({
    //            user: process.env.POSTGRES_USER,
    //            password: process.env.POSTGRES_PASSWORD,
    //            host: 'localhost',
...
app.post('/forgotPassword', function (req, res, next) {
    async.waterfall([
        ...
        function (token, done) {
            var user = usersBeforeConfirmation.find(user => user.email === req.body.email);
            if (!user) {
...
app.post('/reset/:token', async function (req, res) {
    ...
    async.waterfall([
        function (done) {
            ...
            if (req.body.password === req.body.confirm) {
                ...
                var user = usersBeforeConfirmation.find(user => user.email === resetRequest.email);
                done(null, user);
            } else {
        ...
        function (user, done) {
            ...
            smtpTransport.sendMail(mailOptions, function (err) {
                console.log('Success! Your password has been changed.'); // change to flash message
                done(err);
            });
        }
    ], function (err) {
    ...
    try {
        ...
        var indexOfUser = usersBeforeConfirmation.findIndex(user => user.email === resetRequest.email);
        usersBeforeConfirmation[indexOfUser].password = hashedPassword;
        res.redirect('/index');
    } catch {
        res.status(500).send();
    }
